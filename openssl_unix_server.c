	...
	/*----------------------------------------------------------------*/
  	...
  	SSL_library_init();
    	...
    	/* Load the error strings for SSL & CRYPTO APIs */
      	...
      	/* Create a SSL_METHOD structure (choose a SSL/TLS protocol version). We
	 * can also specify SSLv3_server_method() here and SSLv3_client_method in
	 * client program
	 */
     	meth = SSLv3_method();
 	...
 	/* Create a SSL_CTX structure */
        ...
        if (SSL_CTX_use_PrivateKey_file(ctx, RSA_SERVER_KEY, SSL_FILETYPE_PEM) <= 0) {
      	...
      	if(verify_client == ON) {
		/* Set that we require verification of client certificates when
		 * SSL handshake is happening.
		 */
